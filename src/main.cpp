...
void setup() {
  ...
  // --- RECUPERAÇÃO BASEADA EM DEADLINE ---
  if (LittleFS.exists("/status.txt")) {
    ...
    if (f && f.size() > 0) {
      ...
      if (p1 != -1 && p2 != -1 && p3 != -1) {
          ...
          // Só recupera se o fim ainda não passou
...
void loop() {
  ...
  if (experimento.isRunning()) {
    ...
    if (!experimento.isPaused() && (millis() - ultimaAmostragem >= experimento.getIntervalo())) {
      ...
      snprintf(logBuffer, sizeof(logBuffer), "%s;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%s;AUTOMATICO",
      ...
      // Salva Checkpoint (Arquivo;Contador;UnixFim;Pausado)
      File f = LittleFS.open("/status.txt", "w");
      if(f) {
        f.printf("%s;%lu;%u;%d", nomeArquivoSDAtual.c_str(), experimento.getContador(), experimento.getUnixFim(), experimento.isPaused() ? 1 : 0);
        f.close();
      }
    }
